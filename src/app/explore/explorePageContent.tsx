...
export default function ExplorePageContent() {
  ...
  }, [score, isQuizMode, router, setIsValid, setQuizScore]);
  ...
  const onHoverCountry = useCallback((countryName: string) => {setHoveredCountry(countryName)}, []);
  const [isTimeUp, setIsTimeUp] = useState(false);

  // Verificar la respuesta del usuario
  const checkAnswer = useCallback(async (countryName: string) => {
      if (isLoading || !currentQuestion || !countryName|| isAnswered || isTimeUp) return;
    ...
    [isLoading, currentQuestion, isAnswered, isTimeUp]
  );
